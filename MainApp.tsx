...
import { PlusIcon, CalendarDaysIcon, ListBulletIcon, TrophyIcon, ChartPieIcon, QuestionMarkCircleIcon, BellIcon, BellSlashIcon, Cog6ToothIcon, ArrowUpTrayIcon, SparklesIcon, MedalIcon, SearchIcon, UsersIcon } from './components/Icons';
  ...
  return (
    <div className="bg-bg-primary text-text-primary transition-colors duration-300 min-h-screen">
      ...
      <div className="container mx-auto px-2 sm:px-4 py-8">
        <header className="mb-10 text-center">
          <h1 className="text-4xl sm:text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-[var(--accent-400)] to-[var(--accent-300)]">
           ...
           <div className="flex justify-center items-center text-text-secondary mt-3 text-md sm:text-lg">
             <AvatarDisplay avatarId={currentUser?.avatarId} className="w-9 h-9 mr-3" />
             <span>Olá, <strong className="text-text-primary">{currentUser?.username}</strong>! Organize, acompanhe e avalie seus animes favoritos!</span>
           </div>
        </header>

        <div className="mb-8 p-3 sm:p-4 bg-surface-primary rounded-lg shadow-custom-xl flex flex-col md:flex-row justify-between items-center gap-3 sm:gap-4">
          <div className="flex-grow w-full md:w-auto">
              <input 
                  ...
                  className="w-full p-3 bg-surface-secondary border border-border-primary rounded-md focus:ring-2 focus:ring-accent-ring focus:border-accent-border outline-none transition-colors placeholder-text-tertiary text-text-primary"
          ...
          <div className="flex-grow w-full md:w-auto">
              <select 
                  ...
                  className="w-full p-3 bg-surface-secondary border border-border-primary rounded-md focus:ring-2 focus:ring-accent-ring focus:border-accent-border outline-none transition-colors text-text-primary"
                  aria-label="Filtrar por status"
                  disabled={viewMode === 'discover' || viewMode === 'social' || viewMode === 'profile'}

              >
                  <option value="ALL" className="text-text-primary bg-surface-secondary">Todos os Status</option>
                  {Object.values(AnimeStatus).map(s => <option key={s} value={s} className="text-text-primary bg-surface-secondary">{s}</option>)}
              </select>
          </div>
          <div className="flex gap-2 w-full md:w-auto items-stretch overflow-x-auto pb-2 md:pb-0">
              {renderNotificationButton()}
              <button
                  onClick={() => { setViewMode('list'); setListSubView('all');}}
                  className={viewButtonClass('list', viewMode)}
                  aria-pressed={viewMode === 'list'}
                  aria-label="Visualizar em lista"
                  title="Lista"
              >
                  <ListBulletIcon className="w-5 h-5" opticalSize={20}/>
              </button>
              <button
                  onClick={() => setViewMode('discover')}
                  className={viewButtonClass('discover', viewMode)}
                  aria-pressed={viewMode === 'discover'}
                  aria-label="Descobrir novos animes"
                  title="Descobrir"
              >
                  <SearchIcon className="w-5 h-5" opticalSize={20}/>
              </button>
              <button
                  onClick={() => setViewMode('social')}
                  className={viewButtonClass('social', viewMode)}
                  aria-pressed={viewMode === 'social'}
                  aria-label="Visualizar hub social"
                  title="Social"
              >
                  <UsersIcon className="w-5 h-5" opticalSize={20}/>
              </button>
              <button
                  onClick={() => setViewMode('calendar')}
                  className={viewButtonClass('calendar', viewMode)}
                  aria-pressed={viewMode === 'calendar'}
                  aria-label="Visualizar em calendário"
                  title="Calendário"
              >
                  <CalendarDaysIcon className="w-5 h-5" opticalSize={20}/>
              </button>
              <button
                  onClick={() => setViewMode('ranking')}
                  className={viewButtonClass('ranking', viewMode)}
                  aria-pressed={viewMode === 'ranking'}
                  aria-label="Visualizar ranking"
                  title="Ranking"
              >
                  <TrophyIcon className="w-5 h-5" opticalSize={20}/>
              </button>
              <button
                  onClick={() => setViewMode('stats')}
                  className={viewButtonClass('stats', viewMode)}
                  aria-pressed={viewMode === 'stats'}
                  aria-label="Visualizar estatísticas"
                  title="Estatísticas"
              >
                  <ChartPieIcon className="w-5 h-5" opticalSize={20}/>
              </button>
              <button
                  onClick={() => setViewMode('achievements')}
                  className={viewButtonClass('achievements', viewMode)}
                  aria-pressed={viewMode === 'achievements'}
                  aria-label="Visualizar conquistas"
                  title="Conquistas"
              >
                  <MedalIcon className="w-5 h-5" opticalSize={20}/>
              </button>
              <button
                  onClick={() => setIsSettingsModalOpen(true)} 
                  className="p-3 bg-surface-secondary hover:bg-surface-hover text-text-secondary rounded-md transition-colors flex items-center justify-center"
                  title="Configurações"
                  aria-label="Abrir configurações"
              >
                  <Cog6ToothIcon className="w-5 h-5" opticalSize={20}/>
              </button>
          </div>
          <button
            onClick={openAddModal}
            className="w-full md:w-auto flex-shrink-0 bg-accent-cta hover:bg-accent-cta-hover text-white font-semibold py-3 px-4 rounded-md shadow-custom-md hover:shadow-custom-lg transition-all duration-150 ease-in-out transform hover:scale-105 flex items-center justify-center space-x-2"
      ...
      {(viewMode !== 'discover' && viewMode !== 'social' && viewMode !== 'profile') && (
        ...
        <Modal
          isOpen={isModalOpen}
          onClose={() => { setIsModalOpen(false); setEditingAnime(null); }}
